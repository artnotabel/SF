...
print('              Морской Бой!    ')
print('---------------------------------------------------------')
time.sleep(2)   #задержка - эффект "покадрового" вывода информации
...
time.sleep(2)   #задержка - эффект "покадрового" вывода информации
print('Стреляем только в клетки со знаком "точка!"')
print('ПОЕХАЛИ:')
...
class Board:
    ...
    def contour(self, ship, sp=False): #контуры кораблей, обозначение точками их контуров
        n = [    #комбинация относительных индексов каждой взятой точки и точек  рядом с ней
            (-1, -1), (-1, 0), (-1, 1),
            (0, -1), (0, 0), (0, 1),
            (1, -1), (1, 0), (1, 1)
        ...
        for d in ship.dots:
            for dx, dy in n:
                ...
                if not (self.out(now)) and now not in self.busy:
                    if sp:
                        self.field[now.x][now.y] = "*" #контуры уничтоженных кораблей - такой знак
    ...
    def __str__(self): #отображаем само игровой поле с помощью "конструктора"
        ...
        if self.hid:  #в скрытом поле Компьютера изначально ставим только "."
            din  = din.replace("■", ".")
        return din
    ...
    def shot(self, d):
        ...
        for ship in self.ships:
            if d in ship.dots:
                ...
                self.field[d.x][d.y] = "X"   #частичное или полное уничтожение корабля - такой знак
                if ship.lives == 0:
                    self.count += 1
                    self.contour(ship, sp=True)
                    time.sleep(2) #эффект выстрела
                    print('Корабль Убит! Вокруг него стрелять нельзя!')
...
class Main:  #установка основных параметров самой игры
    ...
    def game(self):  #основной игровой цикл
        ...
        while True:
            ...
            time.sleep(2)    #эффект выплывания досок - поочерёдно
            print(self.us.board)
            print('-' * 27)
            ...
            if num % 2 == 0:
                ...
                time.sleep(2)   #эффект произведённого выстрела
                repeat = self.us.move()
            else:
                print('-' * 27)
                time.sleep(1)
                print('Ход Компьютера:  ')
                time.sleep(2)   #эффект "обдумывания хода" Компьютером
                repeat = self.ai.move()
            if repeat:
